...
const DashboardHeader = ({ open, setOpen, mobileOpen, setMobileOpen }) => {
  ...
  return (
    <div>
      <header className={`flex items-center lg:h-20 md:h-20 h-16 ${open === true ? "z-50" : "px-2"} bg-white border-b`}>
        <button className="mx-2 px-4 hidden sm:block" onClick={() => setOpen(open === true ? false : true)}>
          {open ? (
        ...
        {mobileOpen ? (
          <div onClick={() => setMobileOpen(mobileOpen === true ? false : true)} className="bg-[#FEE2E2] text-[#DC2626] h-8 w-8 p-1 rounded-full duration-300 flex flex-col justify-center items-center">
            <VscClose className="text-2xl" />
          </div>
        ) : (
          <div className="h-8 w-8 p-1 rounded-full duration-300 flex flex-col justify-center items-center">
            <button onClick={() => setMobileOpen(mobileOpen === true ? false : true)} className="block sm:hidden relative flex-shrink-0 p-2 mr-2 text-gray-600 hover:bg-[#DBEAFE] hover:text-gray-800 focus:bg-[#DBEAFE] focus:text-gray-800 rounded-full duration-200">
              <svg
                aria-hidden="true"
                fill="none"
                viewBox="0 0 24 24"
          ...
          <div className="lg:border-l md:border-l border-l-0 pl-2 space-x-1">
            ...
            {/* ) : ( */}
              <button className="relative p-2">
                <Link to="/login">
                  <CgLogIn className="text-3xl bg-[#DBEAFE] text-[#2563EB] p-1 flex flex-col justify-center items-center rounded-full" />
                </Link>
              </button>
            {/* )} */}
          </div>
